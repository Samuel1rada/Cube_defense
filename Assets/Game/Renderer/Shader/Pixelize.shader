...
Shader "Hidden/Pixelize"
{
    ...
    SubShader
    {
        ...
        /*Pass
        {
            ...
            {
                // Sample depth
                float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_point_clamp, IN.uv);

                // Calculate depth difference to detect edges
                float depthDifference = fwidth(depth);

                // Highlight edges with the desired color
                fixed4 edgeColor = _EdgeColor;
                return edgeColor * depthDifference;
            }
                ENDHLSL
        }*/
    }
